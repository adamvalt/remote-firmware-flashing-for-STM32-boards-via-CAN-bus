                                        ...
                                        uint32_t *erroneousMap)
{
    ...
    for (uint8_t i = 0; i < mappedObjectsCount; i++) {
...
#endif
        {
            uint8_t *odDataPointer = OD_IO.stream.dataOrig;
            for (uint8_t j = pdoDataStart; j < pdoDataLength; j++) {
                if(OD_IO.stream.attribute & ODA_TYPE_FLOAT)
                {
                    PDO->isVarFloat[j] = 1;
                }
                else
                     ...
                     bool_t syncWas)
{
    ...
    if (PDO->valid && NMTisOperational
        ...
        while (CO_FLAG_READ(RPDO->CANrxNew[bufNo])) {
            ...
            uint8_t buff[4];
            uint8_t buffSize = 0;
            uint8_t floatStartIndex = 0;
            for (uint8_t i = 0; i < PDO->dataLength; i++) {
                if(PDO->isVarFloat[i]) {
                    if(buffSize == 0) {
                        floatStartIndex = i;
                    }
                    buff[buffSize++] = dataRPDO[i];
                    if(buffSize == 4) {
                         int32_t intVar = *(int32_t *) buff;
                         float32_t floatVar = intVar / FLOAT_DECIMAL_PLACES;
                         for(uint8_t j = 0; j < 4; j++) {
                             *PDO->mapPointer[floatStartIndex + j] = ((uint8_t *) &floatVar)[j];
                         }
                         floatStartIndex = 0;
                         buffSize = 0;
                    }
...
static CO_ReturnError_t CO_TPDOsend(CO_TPDO_t *TPDO) {
    ...
    for (uint8_t i = 0; i < PDO->dataLength; i++) {
        if(PDO->isVarFloat[i]) {
            ...
            buff[buffSize++] = *PDO->mapPointer[i];
            if(buffSize == 4) {
                float32_t floatVar = (*(float32_t *) buff);
                int32_t intVar = (int32_t) (floatVar * FLOAT_DECIMAL_PLACES);

                for(uint8_t j = 0; j < 4; j++) {
                    dataTPDO[floatStartIndex + j] = ((uint8_t *) &intVar)[j];
                }
                floatStartIndex = 0;
                buffSize = 0;
            }

...
#define FREE_MEMORY_SIZE ((uint16_t) (SECTOR_SIZE * 5))
...
void loop_1000hz_bootloader(){
    ...
    if (CanOpen->SDOserver->finished && CanOpen->SDOserver->index == 0x6003) {
        ...
        if (is_firmware_in_flash() && is_firmware_crc_ok()) {
            ...
            HAL_FLASH_Lock();
            Flash_Erase(&flash, BOOT_ADDR, FREE_MEMORY_SIZE / 4);
            HAL_FLASH_Unlock();
        }
        fw_uploading = false;
...
void save_buffer_data_to_flash() {
    ...
    Flash_Write_Data_DWORD(&flash, current_address, (uint64_t *) sector_buffer, flash_bytes_count / 8);
